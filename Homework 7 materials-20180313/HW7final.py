...
import csv
"""
These variables are used for referrencing placement in the lists of grades that the 
functions create.
"""
...
def has_trouble(troadd):
    """
    Uses the global variables to sift through our trouble grades
    """
...
def has_concern(conadd):
    """ 
    Uses the global variables list to sift through concerning grades list
    """
    conresult = []
...
def compare(concern, trouble):
    """
    Checks in the concern and trouble lists and makes sure that any trouble grades
    are not repeated in the concern list
    """
...
def add_tr(line):
    """
    creates the list of grades for each student to be analyzed in the
    has_trouble function
    """
    troadd = []
    for x in line[2:]:
        troadd.append(int(x))
...
def student_name():
    ...
    what should be added depending on grades. Then compiles a report
    and spits it out into a .txt document.
    """
